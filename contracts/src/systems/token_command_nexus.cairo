...
mod token_CommandNexus {    
    ...
    //
    #[abi(embed_v0)]
    impl TokenCommandNexusPublicImpl of super::ITokenCommandNexusPublic<ContractState> {
    ...
    impl TokenCommandNexusInternalImpl of TokenCommandNexusInternalTrait {
        fn format_name(self: @ContractState, token_id: u256, CommandNexus: CommandNexus) -> ByteArray {
            let name: ByteArray = if (CommandNexus.name != '') { CommandNexus.name.to_byte_array() } else { "CommandNexus" };
            (format!("{} #{}", name, token_id))
        }
        
        fn format_description(self: @ContractState, token_id: u256, _CommandNexus: CommandNexus) -> ByteArray {
            (format!("Command Nexus - Dominate #{}", token_id))
        }
        
        fn format_image(self: @ContractState, CommandNexus: CommandNexus, variant: ByteArray) -> ByteArray {
